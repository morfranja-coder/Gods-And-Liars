...
func test_only_blocking_match_phases_have_timeouts() -> void:
	...
	assert_int(PhaseTimeoutPolicy.timeout_ms_for_phase(GameManager.MatchPhase.GOD_INTRO)).is_equal(14000)
	assert_int(PhaseTimeoutPolicy.timeout_ms_for_phase(GameManager.MatchPhase.NIGHT_START)).is_equal(3000)
	assert_int(PhaseTimeoutPolicy.timeout_ms_for_phase(GameManager.MatchPhase.HERETIC_ACTION)).is_equal(25000)
	assert_int(PhaseTimeoutPolicy.timeout_ms_for_phase(GameManager.MatchPhase.HEALER_ACTION)).is_equal(10000)
